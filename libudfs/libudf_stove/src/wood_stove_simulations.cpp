...
static real n_cells = 2100; /* Only accounting for pyrolysis on the surface, change this when refining or coarsening the mesh */
...
static int fluid_ID = 430; /* ID of fluid zone */
static int solid_ID = 454; /* ID of solid zone */
static int fluid_interface_ID = 769; /* ID of fluid interface */
static int solid_interface_ID = 776; /* ID of solid interface */
//static float solid_interface_ID_begin = 2.0; /* begin ID of wood surfaces */
//static float solid_interface_ID_end = 81.0; /* end ID of wood surfaces */
//static int step_size_solid_bounds = (solid_interface_ID_end - solid_interface_ID_begin) + 1; /* step size of wood boundaries */
//static float fluid_interface_ID_begin = 504.0; /* begin ID of wood surface fluid */
//static float fluid_interface_ID_end = 583.0; /* end ID of wood surface fluid */
//static int step_size_fluid_bounds = (fluid_interface_ID_end - fluid_interface_ID_begin) + 1; /* step size of wood boundaries */
...
/* result of index search in solid zone array */
...
DEFINE_INIT(density_initial, domain)
{
	...
	thread_loop_c(t, domain) /*loop over cells in the zone*/
	{
		...
		//Message0("The thread ID is %d\n", thread_id);
		//if (thread_id != fluid_ID) /* identify whether the zone is solid or not*/
		//{
			//begin_c_loop(c, t)
			//{
			...
			//	/* uncomment this chunk to use constant specific heat */
			//	if (C_R(c, t) <= rho_crit)
			//	{
...
DEFINE_PROPERTY(rho_wood, c, t)   /* actual wood density profile computation*/
{
	...
	real Tref = 288.16;
	/* uncomment these definitions to use variable specific heats */
	//Cp_cellulose = 1000 * (-0.02951 + (0.00436 * temp)); /* cellulose specific heat */
	//sensible_cellulose = 1000 * (-0.02951 * (temp - Tref) + (0.00436 * 0.5 * (pow(temp, 2) - pow(Tref, 2)))); /* sensible enthalpy */
	//Cp_hemicellulose = 1000 * (-0.009453 + (0.004165 * temp)); /* hemicellulose specific heat */
	//sensible_hemicellulose = 1000 * (-0.009453 * (temp - Tref) + (0.004165 * 0.5 * (pow(temp, 2) - pow(Tref, 2)))); /* sensible enthalpy */
	//Cp_lignin = 1000 * (0.063517 + (0.0039783 * temp)); /* lignin specific heat */
	//sensible_lignin = 1000 * (0.063517 * (temp - Tref) + (0.0039783 * 0.5 * (pow(temp, 2) - pow(Tref, 2)))); /* sensible enthalpy */
	//Cp_char = 1000 * (1.39 + (0.00036 * temp)); /* char specific heat */
	//sensible_char = 1000 * (1.39 * (temp - Tref) + (0.00036 * 0.5 * (pow(temp, 2) - pow(Tref, 2)))); /* sensible enthalpy */

	if (C_UDMI(c, t, 2) == 0)   /*UDM indicating if the density falls within the critical density cut-off region*/
	{
		/*rho_w = (get_density_from_T(temp)) * initial_density; /
		if (rho_w > initial_density)
			rho_w = initial_density;
		if (rho_w < final_density)
			rho_w = final_density;*/
			//if (flow_time == 0)
			//{
				//rho_w = initial_density;
			...
			//else
			//{
				//temp_prev = C_T_M1(c, t); /* previous time step temperature */
				//rho_w_prev = C_R_M1(c, t); /* previous time step density */
				//rho_w = rho_w_prev - ((get_dtg_from_T(temp)) * initial_density * (temp - temp_prev));
	...
	/* apparent density */
	//massfrac_hemicellulose = C_UDMI(c, t, 13) / (1 - f_res);
	//massfrac_cellulose = C_UDMI(c, t, 14) / (1 - f_res);
	//massfrac_lignin = C_UDMI(c, t, 15) / (1 - f_res);
	//massfrac_char = 1 - massfrac_hemicellulose - massfrac_cellulose - massfrac_lignin;
	//Cp_total = ((massfrac_hemicellulose * Cp_hemicellulose) + (massfrac_cellulose * Cp_cellulose) + (massfrac_lignin * Cp_lignin) + (massfrac_char * Cp_char)); /* sensible specific heat */
	//C_UDMI(c, t, 16) = Cp_total;
	//sensible_total = ((massfrac_hemicellulose * sensible_hemicellulose) + (massfrac_cellulose * sensible_cellulose) + (massfrac_lignin * sensible_lignin) + (massfrac_char * sensible_char)) / (2310 * (temp - Tref)); /* normalized sensible specific heat */
	//C_UDMI(c, t, 17) = sensible_total;
	/* uncomment this portion for constant specific heat */
	//if (flow_time > 0) /* for transient simulations, in order for density to not increase with time */
	//{
		//if (rho_w > C_R_M1(c, t)) /* density should not increase with time */
		//{
			//rho_w = C_R_M1(c, t);
		//}
	//}
	...
	c_face_loop(c, t, n)
	{
		...
		{
			F_UDMI(f, tf, 19) = rho_w; /* store density of the boundary cells */
		}
...
/* debugging function */
//DEFINE_ON_DEMAND(test_fn)
//{
	//cell_t c; /* cell in the domain */
	//Domain* domain;
	//domain = Get_Domain(1);
	//Thread* t;
	//int n = 0;
	...
	//float* solid_interface_IDs = populate_linear_space(step_size_solid_bounds, solid_interface_ID_begin, solid_interface_ID_end); /* array containing solid interface IDs */
	//Message0("Print out the interface ID arrays:\n");
	//int size = sizeof(solid_interface_IDs) / sizeof(solid_interface_IDs[0]);
	//for (int i = 0; i < size; i++) 
	//{
		//Message0("%f\n", solid_interface_IDs[i]);
	//}

	//thread_loop_c(t, domain)
	//{
		//if (THREAD_ID(t) != fluid_ID)
		//{
			//begin_c_loop(c, t)
			//{
				//c_face_loop(c, t, n)
				//{
					...
					//int index = search_array(solid_interface_IDs, step_size_solid_bounds, THREAD_ID(tf)); /* search whether the curret solid thread belongs to the solid interface */
					//if (index != 1)
						//printf("The solid interface ID is %d\n", THREAD_ID(tf));
				//}
			//}
			//end_c_loop(c, t)
		//}
...
/* debugging function */
//DEFINE_EXECUTE_AT_END(test_fn_2)
//{
...
//#if !RP_HOST
	...
	//{
		...
		//if (thread_id == solid_domain_ID) /* identify whether the zone is solid or not*/
		//{
			//begin_c_loop(c, t)
			//{
				//C_UDMI(c, t, 3) = C_T(c, t); /* measure temperature */
			//}
			//end_c_loop(c, t)
		//}
	//}
	...
	thread_loop_c(t, domain) /* loop through all the threads in the domain */
	{
		...
		if (thread_id == solid_ID) /* identify whether the zone is solid or not*/
		{
			begin_c_loop(c, t)
			{
				...
				//if (C_R(c, t) <= rho_crit)
				//if (C_R(c, t) / C_UDMI(c, t, 17) <= rho_crit)
				//{
					//C_UDMI(c, t, 2) = 1; /*update cut-off criteria for the apparent density */
