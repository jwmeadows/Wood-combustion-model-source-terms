...
These pseudo-component mass loss rates at every finite volume computatonal cell are recorded using the UDMs ```C_UDMI(c, t, 63)```, ```C_UDMI(c, t, 64)```, and ```C_UDMI(c, t, 65)```. These UDMs are then used in various ```DEFINE_SOURCE``` macros for defining the species (methane, carbon dioxide, and carbon monoxide) source terms and pseudocomponent energy of decomposition terms. The relationships between the species production rates ($$\frac{d m_i}{dt}$$) and the pseudocomponent mass loss rate ($$\frac{d m_j}{dt}$$) are developed using volatile fractions $$\psi_{i, j}$$ as shown in the equation below. The calculation of species production rates involve the computation of mass fractions and mole fractions, which is achieved using the ```get_mole_fractions``` function. The rationale for using the volatile fractions is described in the authors' [previous study](https://doi.org/10.1016/j.fuel.2024.133416).
...
The char oxidation source term macros ```DEFINE_SOURCE(char_oxygen_sink, c, t, ds, eqn)```, ```DEFINE_SOURCE(char_co2_flux, c, t, ds, eqn)```, ```DEFINE_SOURCE(char_co_flux, c, t, ds, eqn)```, and ```DEFINE_SOURCE(char_energy_source, c, t, ds, eqn)``` define the species and energy source terms for char oxidation. These source terms were defined based on the parameters and expressions provided by Anca-Couce et al. in [this](https://doi.org/10.1016/j.combustflame.2011.11.015) paper. The char oxidation rate is then defined as follows:

$$
\left(\frac{1}{V}\right)\left(\frac{-d (m_{char, n, k})}{dt} \right) = A_{char} exp\left( -\frac{E_{char}}{R_uT} \right)(0.98 * \rho_{char, 0, k} * e)\left(\frac{(m_{char, 0, k} * \delta^{n}) - m_{ash, k}}{m_{char, 0, k} - m_{ash, k}}\right)^{n_{rxn}} \left(\frac{\chi_{O_{2, k}}}{0.205}\right)^{n_{O_2}}
$$

Hooking both the char oxidation source term and the pyrolysis source term macros may sometimes lead to instability issues and floating point exceptions depending on the scenario that you are simulating. In this aspect, hooking the macros is more of an art than a science and may require multiple iterations to ensure stable solutions. For best results, hook the pyrolysis source term macros alone for a few time steps and hook the char oxidation macros in the later phase of the simulation. In most cases, char oxidation source terms are significant near burn-out and can be neglected during ignition and start-up phases.

In the default state, the code executes partial oxidation of carbon to CO. This reaction has an exothermic energy source term of 9.2 MJ/kg. The CO/CO2 split can be dynamically changed by altering the source terms in the  ```DEFINE_SOURCE(char_oxygen_sink, c, t, ds, eqn)```, ```DEFINE_SOURCE(char_co2_flux, c, t, ds, eqn)```, and ```DEFINE_SOURCE(char_co_flux, c, t, ds, eqn)``` macros. Each time you change the source terms, the energy source term needs to be updated in the ```DEFINE_SOURCE(char_energy_source, c, t, ds, eqn)``` macro.
